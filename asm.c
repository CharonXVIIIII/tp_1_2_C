...
typedef enum TOKEN_TYPE {
    TOK_NUL, TOK_INST, TOK_VALUE = 222
} TOKEN_TYPE;
...
static void err_asm(char* err) {
    fprintf(stderr, "Fichier %s, Ligne %d: %s\n", file_name, nu_ligne, err);
    exit(EXIT_FAILURE);
}
...
static TOKEN tokenize(char* data) {
    ...
    int i = search(s, instructions_dico);
    if (i >= 0) {
...
static void assemble_line(char *line) {
    ...
    // instruction define
    if (t.type == TOK_INST && t.value == SPECIAL_DEFINE) {
        assemble_define();
        return;
    ...
    if (inst.op == SPECIAL_DATA) {
        write_mem(logical_adr, inst.arg);
    } else {
        make_inst(logical_adr, inst);
    }
    logical_adr++;
...
static void test_tokenize() {
    char line[100] = "  set 1234, hello  // Comment ";
...
void assemble(WORD adr, char *p_file_name) {
    char buffer[300];
